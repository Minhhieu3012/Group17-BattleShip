...
from game_logic import Player, GameRoom
...
def handle_message(msg, player, room, conn):
    ...
    if action == 'join':
        ...
        x, y = msg.get('x'), msg.get('y')  # Lấy thông tin vị trí của tàu
        length, direction = msg.get('length'), msg.get('dir')  # Lấy thông tin chiều dài và hướng của tàu
        if None in (x, y, length, direction):
            send_json(conn, {'action': 'error', 'message': 'Missing ship placement data'})
            return player, room
        ...
        # Đặt tất cả tàu từ ships data nếu có
        ships = msg.get('ships', [])
        if ships:
            for ship in ships:
                pos = ship.get('pos')
                ...
                if pos and length:
                    row, col = pos
                    player.board.place_ship(col, row, length, orientation)
        
        player.ready = True
        send_json(conn, {'action': 'ready', 'status': 'confirmed'})
        ...
        send_json(conn, {'action': 'error', 'message': f'Invalid action: {action}'}) 
    
    return player, room

def join_room(msg, conn):
    name = msg.get('name', 'Guest')  # Lấy tên người chơi từ message
    room_id = msg.get('room_id') or str(uuid.uuid4())[:8]  # Lấy room_id từ message hoặc tạo mới
    ...
    if not room.add_player(player):
        send_json(conn, {'action': 'error', 'message': 'Room is full'})
        return None, None
...
def handle_shoot(msg, player, room, conn):
    ...
    x, y = msg.get('x'), msg.get('y')  # Lấy tọa độ bắn từ message
    
    if None in (x, y):
        send_json(conn, {'action': 'error', 'message': 'Invalid shot coordinates'})
        return
    ...
    if room.players[target_idx].board.check_all_sunk():
        ...
        broadcast_room(room, {'action': 'turn_change', 'turn': room.turn})
